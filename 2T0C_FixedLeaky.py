...
class IGZO_2T0C_Array:
    ...
    def update_and_read(self, u_in):
        ...
        for i in range(self.n):
            ckt = Circuit()
            v_wwl_write = 1.0 + 0.5 * u_in

            ckt.add_voltage_source('WWL', 2, 0, func_v_t=lambda t: v_wwl_write if t < T_write else 0.8)
            ...
            # 引入耦合电容：节点 2 (WWL) 到 节点 3 (SN)
            ckt.add_capacitor('C_wwl_sn', 2, 3, 46e-15)
            # 引入读取耦合电容：节点 4 (RBL) 到 节点 3 (SN)
            ...
            final_v_wwl = get_voltage(ckt, res, 2)[-1]
            final_v_rbl = get_voltage(ckt, res, 4)[-1]
            #保存状态：节点 3 (SN), 节点 2 (WWL), 节点 4 (RBL)
            self.device_states[i] = {3: final_v_sn, 2: final_v_wwl, 4: final_v_rbl}
...
try:
    ...
    print('Warning: mackey_glass.npy not found, using placeholder.')
